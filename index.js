...
function processSum(numberList,callback) {
  return callback(numberList.reduce (function(accum , item) {
    return accum + item;
  },0))
}
 ...
 * @returns an array with all the runners' full names in the following format: "JSmith, ohn".
...
const shirts = runners.filter(person =>
...
function tallyUpDonations(runners) {
 return runners.reduce ((sum, persons) => persons.donation + sum, 0);
}
